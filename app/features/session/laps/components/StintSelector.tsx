            ...
            {isStintSelectorOpen && (
                ...
                >
                    {driverStints.map((stintInstance) => (
                        <label className="grid grid-cols-[48px,_128px] gap-2 items-center" key={stintInstance.driver}>
                            <span>{stintInstance.driver}</span>
                            <select
                                className="select select-sm w-full text-end"
                                onChange={(evt) =>
                                    onChange({ driver: stintInstance.driver, stint: Number.parseInt(evt.target.value) })
                                }
